...
class NormGymEnv(gym.Env):
    ...
    def __init__(
        ...
        self.max_dist = np.sqrt(
            self.env.room_width**2 + self.env.room_height**2
        )
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        lidar_proximity = 1.0 - lidar_norm

        if self.stack_dim > 1:
            if self.lidar_stack is None:
                self.lidar_stack = [lidar_proximity.copy() for _ in range(self.stack_dim)]
            else:
                self.lidar_stack.pop(0)
                self.lidar_stack.append(lidar_proximity.copy())
            lidar_stack = np.concatenate(self.lidar_stack, axis=0)
        else:
            lidar_stack = lidar_proximity.astype(np.float32)

        obs = np.concatenate([
            [rho_norm, wrapped_angle / np.pi],
            lidar_stack,
        ]).astype(np.float32)
    ...
    def step(self, action):
        ...
        v_norm = np.clip(action[0], 0.0, 1.0)
        v = float(v_norm * MAX_LIN_VEL)

        w_norm = np.clip(action[1], -1.0, 1.0)
        w = float(w_norm * MAX_ANG_VEL)
